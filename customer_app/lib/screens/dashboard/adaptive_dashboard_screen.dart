...
/// Enhanced dashboard that adapts based on user role (supplier or admin/host)
class AdaptiveDashboardScreen extends ConsumerStatefulWidget {
  const AdaptiveDashboardScreen({super.key});

  @override
  ConsumerState<AdaptiveDashboardScreen> createState() =>
      _AdaptiveDashboardScreenState();
}

class _AdaptiveDashboardScreenState
    extends ConsumerState<AdaptiveDashboardScreen> {
            ...
            SliverList(
              delegate: SliverChildListDelegate([
                if (adminState.isLoading && adminState.data == null)
                  _buildLoadingState()
                else if (adminState.data != null)
                  _buildAdminDashboardContent(adminState.data!)
  ...
  Widget _buildAdminDashboardContent(dynamic data) {
    final theme = Theme.of(context);
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(
                child: StatsCard(
                  ...
                  value: currencyFormat
                      .format(data.platformAnalytics.totalPlatformRevenue),
              ...
              Expanded(
                child: StatsCard(
                  title: 'Active Users',
                  value: data.platformAnalytics.totalActiveUsers.toString(),
                  icon: Icons.people_rounded,
                  iconColor: AppColors.primaryBlue,
                  subtitle:
                      '${data.adminMetrics.userGrowth.totalCustomers} customers',
                ),
              ),
            ],
          ),
          ...
          Row(
            children: [
              Expanded(
                child: StatsCard(
                  title: 'Total Orders',
                  value: data.platformAnalytics.totalOrders.toString(),
              ...
              Expanded(
                child: StatsCard(
                  title: 'System Health',
                  value: data.systemHealth.status == 'healthy' ? '98%' : '85%',
                  icon: Icons.health_and_safety,
                  iconColor: data.systemHealth.status == 'healthy'
                      ? AppColors.success
                      : AppColors.warning,
                  subtitle: data.systemHealth.status,
                ),
              ),
            ],
          ),
          ...
          const SizedBox(height: AppConstants.spacing12),
          _buildRevenueBreakdown(data.adminMetrics.revenueBreakdown),
          ...
          Row(
            ...
            children: [
              Expanded(
                child: _buildUserGrowthCard(data.adminMetrics.userGrowth),
              ),
              const SizedBox(width: AppConstants.spacing12),
              Expanded(
                child: _buildSystemHealthCard(data.systemHealth),
              ),
            ],
          ),
          ...
          // Recent Users and Orders in tabs
          _buildRecentActivityTabs(data.recentUsers, data.recentOrders),
            ...
            children: [
              Expanded(
                child: StatsCard(
                  ...
                  trend: revenue.growthPercentage != null
                      ...
                      : null,
                  isPositiveTrend: revenue.growthPercentage != null &&
              ...
              Expanded(
                child: StatsCard(
                  ...
                  iconColor: AppColors.primaryBlue,
                  subtitle:
                      '${_getOrderCountByStatus(data.ordersByStatus, 'pending')} pending',
                ),
              ),
            ],
          ),
  ...
  Widget _buildRevenueBreakdown(dynamic breakdown) {
    final theme = Theme.of(context);
    final isDark = theme.brightness == Brightness.dark;
    final currencyFormat = NumberFormat.currency(symbol: 'Rs ');
  ...
  Widget _buildUserGrowthCard(dynamic metrics) {
    final theme = Theme.of(context);
    final isDark = theme.brightness == Brightness.dark;
  ...
  Widget _buildGrowthRow(String label, int value, IconData icon, Color color) {
    final theme = Theme.of(context);
      ...
      child: Column(
        ...
        children: [
          ...
          _buildHealthRow(
            ...
            health.status,
            Icons.circle,
            health.status == 'healthy' ? AppColors.success : AppColors.warning,
          ),
          const SizedBox(height: AppConstants.spacing12),
          _buildHealthRow(
            'Uptime',
  ...
  Widget _buildRecentActivityTabs(List<dynamic> users, List<dynamic> orders) {
  ...
  Widget _buildRecentUserCard(dynamic user) {
    final theme = Theme.of(context);
    final isDark = theme.brightness == Brightness.dark;
